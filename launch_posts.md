# drift-lens-monitor Launch Posts
...
So I built **drift-lens-monitor** — an open-source Python tool that detects embedding space drift *before* your metrics degrade.
...
- **Zero infrastructure**: `pip install drift-lens-monitor` → 5 lines of code → done. No cloud, no API keys, no YAML. Snapshots are flat parquet files.

**The demo that sold me on building this:**
The dashboard shows a synthetic 14-day scenario where drift-lens-monitor fires an alert on Day 8. Accuracy doesn't drop until Day 12. That's a **4-day early warning** from 5 lines of code.
...
PyPI: `pip install drift-lens-monitor`
...
**Title:** Show HN: drift-lens-monitor – detect embedding space drift before your metrics do
...
drift-lens-monitor is an open-source Python tool for detecting drift in embedding spaces. The core idea: if you monitor your model's embeddings directly, you can catch distribution shifts days before downstream accuracy degrades.
...
Install: `pip install drift-lens-monitor`
...
**Title:** drift-lens-monitor — open-source tool to monitor your local embedding model in production
...
I built **drift-lens-monitor** to solve this. It monitors the embedding space itself — not accuracy, not loss — and fires an alert when the statistical or topological structure changes.

**Why this matters for local models:**
- You swap from `all-MiniLM-L6-v2` to `nomic-embed-text` → drift-lens-monitor quantifies exactly how different the spaces are
- Your input data distribution shifts over time → drift-lens-monitor catches it before your RAG pipeline starts returning garbage
- You fine-tune your model → drift-lens-monitor tells you if the new embeddings have collapsed or diverged
...
Runs entirely local. No API keys, no cloud. Just `pip install drift-lens-monitor`.
...
I built drift-lens-monitor: open-source embedding drift detection that fires BEFORE your metrics drop.
...
drift-lens-monitor watches the leading indicator: the embedding space itself.
...
- [ ] Verify `pip install drift-lens-monitor` works from PyPI
